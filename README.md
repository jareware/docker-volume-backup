...
You probably don't want to keep all backups forever. A more common strategy is to hold onto a few recent ones, and remove older ones as they become irrelevant. There's a built-in support for this in `docker-volume-backup` provided by [rotate-backups](https://github.com/xolox/python-rotate-backups). If you transfer your backups via SCP to a remote host, you can trigger the rotate-backups script by means of setting the environmental variable `POST_SCP_COMMAND`.
...
```
We kindly refer to the documentation of rotate-backups for [more information on custom configurations](https://rotate-backups.readthedocs.io/en/latest/readme.html#configuration-files).
